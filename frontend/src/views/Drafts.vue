      ...
      <table class="mail-table">
        ...
        <tbody>
          <tr
            ...
            :key="m.id"
            class="mail-row"
            :class="{ 'mail-important': m.important }"
          ...
          >
            ...
            <td @click.stop>
              <div class="action-btns">
                <button class="btn-action btn-edit" @click="router.push(`/compose?draftId=${m.id}`)">
                  <Edit :size="13" />
                  编辑
                </button>
                <button
          ...
          <tr v-if="!loaded && mails.length === 0">
            <td colspan="4" class="skeleton-state">
...
import api from '../api'
import { Search, Star, Paperclip, Trash2, Flag, Edit, FileText } from 'lucide-vue-next'
import { useMailStore } from '../stores/mail'

defineOptions({ name: 'DraftsPage' })
...
async function loadMails() {
  ...
  // 首次加载优先用预取缓存
  if (!loaded.value && mailStore.drafts) {
    mails.value = mailStore.drafts.list
    total.value = mailStore.drafts.total
    loaded.value = true
    mailStore.drafts = null
  }
  const res = await api.get('/mail/drafts', { params: { page: page.value, size } })
  if (res.code === 0) {
...
async function doSearch() {
  ...
  const res = await api.get('/search', { params: { keyword: keyword.value, box: 'drafts', searchIn: searchIn.value, page: page.value, size } })
  if (res.code === 0) {
...
.skeleton-state { padding: 0 !important; border-bottom: none !important; }
.skeleton-row { display: flex; align-items: center; gap: 16px; padding: 14px 20px; border-bottom: 1px solid var(--border); }
.skeleton { background: linear-gradient(90deg, var(--border) 25%, var(--muted-light, #e5e7eb) 50%, var(--border) 75%); background-size: 200% 100%; animation: shimmer 1.5s infinite; border-radius: 6px; }
.skeleton-icon { width: 16px; height: 16px; border-radius: 50%; flex-shrink: 0; }
.skeleton-name { width: 120px; height: 14px; flex-shrink: 0; }
.skeleton-subject { flex: 1; height: 14px; }
.skeleton-time { width: 100px; height: 14px; flex-shrink: 0; }
