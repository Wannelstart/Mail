          ...
          >
            ...
            <td>
              <div class="receiver-cell">
                <div class="avatar">
                  {{ getInitial(m.receiverUsername || m.receiverEmail) }}
                </div>
                <span class="receiver-name">
                  {{ m.receiverUsername || m.receiverEmail }}
                </span>
              </div>
            </td>
            <td>
...
async function loadMails() {
  ...
  // 首次加载优先用预取缓存
  if (!loaded.value && mailStore.sent) {
    mails.value = mailStore.sent.list
    total.value = mailStore.sent.total
    loaded.value = true
    mailStore.sent = null
  }
  const res = await api.get('/mail/sent', { params: { page: page.value, size } })
  if (res.code === 0) {
...
async function doSearch() {
  ...
  const res = await api.get('/search', { params: { keyword: keyword.value, box: 'sent', searchIn: searchIn.value, page: page.value, size } })
  if (res.code === 0) {
